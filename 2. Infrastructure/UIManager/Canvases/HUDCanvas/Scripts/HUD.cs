...
namespace EGS
{
    ...
    public class HUD : MonoBehaviour
    {
        ...
        public void ActivatePlayerCard(int index, Health health, WeaponSelector weaponSelector)
        {
            switch (index)
            {
                case 0:
                    InitCard(_player0Card, health, weaponSelector);
                    break;
                case 1:
                    InitCard(_player1Card, health, weaponSelector);
                    break;
                case 2:
                    InitCard(_player2Card, health, weaponSelector);
                    break;
                case 3:
                    InitCard(_player3Card, health, weaponSelector);
                    break;
                default:
        ...
        private void InitCard(GameObject card, Health health, WeaponSelector weaponSelector)
        {
            card.SetActive(true);
            card.GetComponentInChildren<UIHealthBar>().HealthSource = health;
            card.GetComponentInChildren<UIWeaponInfo>().WeaponSelector = weaponSelector;
        }
        ...
        private void TerminateCard(GameObject card)
        {
            if (card != null)
            {
                ...
                card.GetComponentInChildren<UIWeaponInfo>().WeaponSelector = null;
